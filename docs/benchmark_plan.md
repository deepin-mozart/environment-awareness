...
> **注**：RSS 阈值较高（150/200MB）是因为 Qt5 + X11 + QDBus 加载后固定占用 ~114MB（实测 S4 规模）。这不是业务逻辑的内存消耗，而是 C++ Qt 框架本身的堆分配。阈值核心目的是检测**增长趋势**（内存泄漏），而非绝对值。若启动 RSS 即超过 150MB 或运行期间持续线性增长不收敛，需用 valgrind/heaptrack 定位泄漏。
