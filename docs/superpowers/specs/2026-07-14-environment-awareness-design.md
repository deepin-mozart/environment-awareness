...
| 方法 | 参数 | 返回 | 说明 |
|---|---|---|---|
| `GetActiveWindow` | — | `a{sv}` | 当前活跃窗口：title, app_name, pid, wclass |
...
**数据源**：
- 主：KWin `org.kde.KWin` D-Bus 接口，监听窗口创建/销毁/激活信号
- 备：X11 `XGetInputFocus` 轮询 + `XSelectInput` 事件通知
...
**实现要点**：
- 通过 `QDBusConnection::connect` 订阅 KWin 信号
- 窗口信息通过 `_NET_WM_NAME`、`WM_CLASS`、`_NET_WM_PID` 获取
- 焦点切换时自动结束上一个 app_session，开始新的
- 轮询 fallback 频率：2秒/次（仅当 D-Bus 信号不可用时）
...
- 文本脱敏后存储，超长截断（200字符预览 + 完整内容哈希）
- 图片只记录尺寸和哈希
- 去重：内容哈希相同且 < 5秒 不重复记录

### 5.4 InputSensor（键鼠事件）

**数据源**：X11 `XRecord` 扩展或 `/dev/input/event*`
...
**实现要点**：
- X11 `XRecord` 范围：`XRecordAllClients`
...
- 当前标签页：通过浏览器 D-Bus 接口或 X11 窗口属性获取当前活跃标签页 URL
...
**实现要点**：
- 历史数据库为浏览器独占锁，**只读 + WAL 模式**打开，避免锁冲突
- 定时轮询历史数据库（默认 5 秒），增量读取自上次以来的新记录（通过 `last_visit_time` 字段）
- URL 域名提取 + 分类（搜索/社交/开发/购物等），存入 metadata 便于后续分析
- 隐私：URL 中的查询参数脱敏（移除 token/session/key 等敏感参数），URL 本身不脱敏
- 当 deepin-browser 提供 D-Bus 接口时，优先使用实时信号，轮询作为 fallback
...
```json
{
    ...
    "sensors": {
        ...
        "system": { "enabled": true, "alert_battery": 20, "alert_disk": 90, "alert_cpu": 90 },
        "browser": {
            "enabled": true,
